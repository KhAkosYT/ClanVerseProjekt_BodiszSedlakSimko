...
app.post('/api/login', auth, async (req, res, next) => {
    ...
    try {
        ...
        if(!user || user.password !== password){
            return Code401(null, req, res, next, "Hibás felhasznalonev vagy jelszó.");
        }
        res.status(200).json({ message: "Sikeres bejelentkezés.", userId: user.id });
    } catch (error) {
        return Code500(error, req, res, next);
    }
});
